...
/**
 * `justReset` arrives as a prop from the page rather than being read here with
 * `useSearchParams`. That hook would force this whole subtree to render on the
 * client only, so the sign-in form would flash empty before hydrating.
 */
export default function SignInForm({ justReset = false }) {
    ...
    <main className="mx-auto flex w-full max-w-md flex-1 flex-col px-6 pb-8">
      ...
      <form action={formAction} className="flex flex-1 flex-col">
        ...
        <div className="mt-6 flex flex-col gap-3">
          ...
          <div>
            {/* The link lives on the label row, which is where people look for it. */}
