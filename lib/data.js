...
export const getSettings = cache(async (userId) => {
  const rows = await sql`
    ...
    returning daily_goal_cents::float8, currency, timezone, rollover_enabled, theme, theme_mode, terms_version
  `;
...
export async function getDayTotal(userId, day) {
  const rows = await sql`
    select coalesce(sum(amount_cents), 0)::float8 as total_cents, count(*)::int as entries
...
export async function getExpensesForDay(userId, day) {
  return sql`
    select id, amount_cents::float8 as amount_cents, category, note,
           spent_on::text as spent_on, created_at
    from public.expenses
    where user_id = ${userId} and spent_on = ${day}
...
export async function getTotals(userId) {
  const rows = await sql`
    select
      coalesce(sum(amount_cents), 0)::float8 as total_cents,
...
export const getActiveChallenge = cache(async (userId) => {
  const rows = await sql`
    select id, kind, cap_cents::float8 as cap_cents,
           starts_on::text as starts_on, ends_on::text as ends_on,
           allowed_slips, target_cents::float8 as target_cents,
           status, ended_on::text as ended_on
...
export async function getCategoryTotals(userId, from, to) {
  return sql`
    select category, sum(amount_cents)::float8 as cents, count(*)::int as entries
    from public.expenses
